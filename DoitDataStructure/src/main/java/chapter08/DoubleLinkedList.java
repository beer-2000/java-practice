...
public class DoubleLinkedList<E> {
    ...
    public void purge(Comparator<? super E> c) {
        Node<E> pointerOuter = head.next;
        Node<E> pointerInner;

        while (pointerOuter != head) {
            pointerInner = pointerOuter.next;
            while (pointerInner != head) {
                if (c.compare(pointerOuter.data, pointerInner.data) == 0) {
                    remove(pointerInner);
                }
